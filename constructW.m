...
%             options.t = mean(mean(D));
              options.t = sqrt(options.t) * mean(mean(D));
...
if strcmpi(options.NeighborMode,'KNN') && (options.k > 0)
    ...
    % If bTrueKNN is set to be 0, then the obtain W is a symmetric matrix,
    % otherwise, it is a true knn.
    if isfield(options,'bTrueKNN') && options.bTrueKNN
        % Doing nothing.
    else
        W = max(W,W'); % Symmetric matrix
    end
